...
def verify_summary(source, summary, api_key, timeout=30):
    report = {"version": 1, "status": "error", "reason": "Verification unavailable"}
    if not api_key:
    ...
    try:
        ...
        return {
            ...
            "checked_at": datetime.now(timezone.utc).isoformat(), "model": "deepseek-chat",
        }
